import { hp, hy, hg, t, plain, verse, type RichChapterContent } from './types';

/**
 * Isaiah 48 — Christ Is Our Peace
 *
 * "Hear ye this, O house of Jacob, which are called by the name of Israel...
 * I am he; I am the first, I also am the last." God speaks to the exiled
 * people. You are called by My name. You bear My name. Yet you have been
 * stubborn and stiff-necked. But I have refined you in the furnace of
 * affliction. I have done this for My name&apos;s sake. And the promise comes:
 * "There is no peace, saith the Lord, unto the wicked." But for those who
 * listen, who are redeemed, who return to God, there is peace that passes
 * understanding. This peace is what Christ came to bring.
 */
export const ISAIAH_48: RichChapterContent = {
  bookSlug: 'isaiah',
  bookName: 'Isaiah',
  chapter: 48,

  estimatedMinutes: { beginner: 1, intermediate: 3, deep: 4 },
  intros: [
    'Isaiah 48 is the conclusion of the book&apos;s section on comfort and restoration. It opens with God addressing the exiled people: you are called by My name. You belong to Me. Yet you have been stubborn. You have had ears but did not hear. You have been appointed to test you in the furnace of affliction. But I have done this for My own name&apos;s sake. I have refined you as silver is refined. And now comes the climax: there is no peace for the wicked. But for those who leave Babylon, who put their trust in God, who return to the Lord, there is peace. Not the peace of comfort or ease, but the peace that comes from being at home in God, from knowing you are safe in His hands. This is the peace that Christ came to give.',
  ],

  sections: [
    {
      ref: 'Isaiah 48:1–22',
      title: 'Called by God&apos;s Name; Refined in Affliction; Promised Peace',
      blocks: [
        {
          kind: 'scripture',
          chapter: 48,
          lines: [
            plain(1, 'Hear ye this, O house of Jacob, which are called by the name of Israel, and are come forth out of the waters of Judah, which swear by the name of the Lord, and make mention of the God of Israel, but not in truth, nor in righteousness.'),
            plain(2, 'For they call themselves of the holy city, and stay themselves upon the God of Israel; The Lord of hosts is his name.'),
            plain(3, 'I have declared the former things from the beginning; and they went forth out of my mouth, and I shewed them; I did them suddenly, and they came to pass.'),
            plain(4, 'Because I knew that thou art obstinate, and thy neck is an iron sinew, and thy brow brass;'),
            plain(5, 'Therefore I have told thee from the beginning; before it came to pass I shewed it thee: lest thou shouldest say, Mine idol hath done them, and my graven image, and my molten image, hath commanded them.'),
            plain(6, 'Thou hast heard, see all this; and will not ye declare it? I have shewed thee new things from this time, even hidden things, and thou didst not know them.'),
          ],
        },
        {
          kind: 'commentary',
          id: 'isaiah48-hidden-things',
          html:
            'God reveals what was hidden, showing things Israel never knew. Yet even faced with proof of God&apos;s knowledge and power, the people remain obstinate. The tone shifts now to Israel&apos;s profound refusal to hear and the graciousness God shows despite their treachery.',
        },
        {
          kind: 'scripture',
          chapter: 48,
          lines: [
            plain(7, 'They are created now, and not from the beginning; even before the day when thou heardest them not; lest thou shouldest say, Behold, I knew them.'),
            plain(8, 'Yea, thou heardest not; yea, thou knewest not; yea, from that time that thine ear was not opened: for I knew that thou wouldest deal very treacherously, and wast called a transgressor from the womb.'),
            plain(9, 'For my name&apos;s sake will I defer mine anger, and for my praise will I refrain for thee, that I cut thee not off.'),
            plain(10, 'Behold, I have refined thee, but not with silver; I have chosen thee in the furnace of affliction.'),
            plain(11, 'For mine own sake, even for mine own sake, will I do it: for how should my name be polluted? and I will not give my glory unto another.'),
          ],
        },
        {
          kind: 'commentary',
          id: 'isaiah48-refining',
          html:
            'The purpose of Israel&apos;s affliction is declared: God has refined Israel in the furnace—not for punishment alone, but for purification. And God acts ultimately for God&apos;s own sake: "My name shall not be polluted." God is bound by God&apos;s own character. God has made promises. God will keep them.',
        },
        {
          kind: 'scripture',
          chapter: 48,
          lines: [
            plain(12, 'Hearken unto me, O Jacob and Israel, my called; I am he; I am the first, I also am the last.'),
            plain(13, 'Mine hand also hath laid the foundation of the earth, and my right hand hath spanned the heavens: when I call unto them, they stand up together.'),
            plain(14, 'All ye, assemble yourselves, and hear; which among them hath declared these things? The Lord hath loved him: he will do his pleasure on Babylon, and his arm shall be on the Chaldeans.'),
            plain(15, 'I, even I, have spoken; yea, I have called him: I have brought him, and he shall make his way prosperous.'),
            plain(16, 'Come ye near unto me, hear ye this; I have not spoken in secret from the beginning; from the time that it was, there am I: and now the Lord God, and his Spirit, hath sent me.'),
          ],
        },
        {
          kind: 'commentary',
          id: 'isaiah48-called',
          html:
            'God calls Israel forward: "I am the first, I also am the last." All history is encompassed in God&apos;s hand. God has loved Cyrus and called him by name. God has made his way prosperous. God speaks not in secret but openly, from the beginning. And God&apos;s Spirit has sent this message. Israel is not left guessing about God&apos;s will.',
        },
        {
          kind: 'scripture',
          chapter: 48,
          lines: [
            plain(17, 'Thus saith the Lord, thy Redeemer, the Holy One of Israel; I am the Lord thy God which teacheth thee to profit, which leadeth thee by the way that thou shouldest go.'),
            plain(18, 'O that thou hadst hearkened to my commandments! then had thy peace been as a river, and thy righteousness as the waves of the sea:'),
            plain(19, 'Thy seed also had been as the sand, and the offspring of thy bowels like the gravel thereof; his name should not have been cut off nor destroyed before me.'),
            plain(20, 'Go ye forth of Babylon, flee ye from the Chaldeans, with a voice of singing declare ye, tell this, utter it even to the end of the earth; say ye, The Lord hath redeemed Jacob.'),
            plain(21, 'And they thirsted not when he led them through the deserts: he caused the waters to flow out of the rock for them: he clave the rock also, and the waters gushed out.'),
            plain(22, 'There is no peace, saith the Lord, unto the wicked.'),
          ],
        },
        {
          kind: 'commentary',
          id: 'isaiah48-called-2',
          html:
            'The chapter opens with identification: "O house of Jacob, which are called by the name of Israel." To be called by God&apos;s name is to belong to Him, to be His people. Yet this people, despite their name and their claims, have not been true. They "swear by the name of the Lord...but not in truth, nor in righteousness." The name is theirs, but not the reality.',
        },
        {
          kind: 'hebrew',
          id: 'isaiah48-kruth',
          title: 'Kruth — "Refined" (Tested, Purified)',
          script: 'כְּרוּת',
          translit: '<strong>Kruth</strong> · refined, tested, purified; literally refined as metal is refined in fire',
          description:
            'God says: "Behold, I have refined thee, but not with silver; I have chosen thee in the furnace of affliction." The affliction of exile is God&apos;s refining fire. It burns away dross. It purifies. This is not punishment alone. This is purification. This is preparation.',
        },
        {
          kind: 'commentary',
          id: 'isaiah48-name-sake',
          html:
            'The key phrase echoes: "For my name&apos;s sake will I defer mine anger...For mine own sake, even for mine own sake, will I do it." God acts, ultimately, not because Israel deserves it, but because God&apos;s own name and glory are at stake. God has made promises. God has taken Israel as His people. If Israel perishes, God&apos;s name is profaned. So God will act to save Israel for God&apos;s own sake.',
        },
        {
          kind: 'commentary',
          id: 'isaiah48-peace-river',
          html:
            'The promise is conditional: "O that thou hadst hearkened to my commandments! then had thy peace been as a river, and thy righteousness as the waves of the sea." If Israel had listened, peace would have been abundant, flowing, endless like a river. But Israel did not listen. So the promise goes unfulfilled—not because God is unwilling, but because Israel is unwilling.',
        },
        {
          kind: 'commentary',
          id: 'isaiah48-no-peace',
          html:
            'The final word is stern: "There is no peace, saith the Lord, unto the wicked." This echoes a theme from earlier in Isaiah. There is no peace for those who turn from God, who refuse His word, who build their lives on rebellion. But for those who hear, who turn, who return to the Lord, there is peace beyond description.',
        },
        {
          kind: 'christ',
          id: 'isaiah48-christ-peace',
          title: 'Christ Connection — Peace Through the Redeemer',
          html:
            'Jesus came as the Prince of Peace. He told His disciples: "Peace I leave with you, my peace I give unto you: not as the world giveth, give I unto you." This is not the absence of conflict. This is the peace that comes from knowing the Redeemer, from being at home in God, from trusting that all things work according to God&apos;s purpose. Christ is the one who fulfills the promise of the Redeemer and brings true peace.',
        },
        {
          kind: 'carry',
          html:
            'The choice before every reader is clear. Will you hearken to God&apos;s commandments and find your peace as a river? Or will you turn from God and find no peace? Will you leave the Babylons of this world—the systems, the powers, the false securities—and return to God? Will you listen to the voice of the Redeemer and find peace? The promise is not to the comfortable or the self-satisfied. The promise is to those who hear and respond.',
        },
        {
          kind: 'reflection',
          id: 'isaiah48-peace-river-2',
          prompt: 'Do you truly long for peace as a river? What would it take for you to hearken to God&apos;s commandments? What false securities, what "Babylons," do you need to leave behind to return to God and find the peace that He offers?',
        },
      ],
    },
  ],

  bottomShare: {
    quote: 'Hear ye this...I am he; I am the first, I also am the last...O that thou hadst hearkened to my commandments! then had thy peace been as a river.',
    snippet: 'A chapter-by-chapter study guide from Learn of Christ.',
    ref: 'Isaiah 48 · Study Guide',
  },

  hasHebrew: true,
};
