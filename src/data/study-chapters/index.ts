 ...
 *      genesis-1.ts) and are listed in RICH_CHAPTER_KEYS below. Each one
 *      is loaded lazily via dynamic import, so a request that only needs
 *      one chapter doesn't pull the whole 56MB tree into the worker.
 *      Hand-authored chapters get the full Genesis 1 template —
 *      verse-by-verse marks, Hebrew/Greek callouts, Christ Connection
 *      boxes, Carry blocks, the works.
 *
 *   2. Every other chapter — anything not in RICH_CHAPTER_KEYS — is
 *      auto-ported from the legacy ChapterContent shape (overview /
 *      themes / questions / christConnection / keyVerse) so the page
 *      renders in the same Genesis-1 visual idiom while we wait for
 *      handcrafted commentary.
 *
 * Result: every study guide on the site uses RichStudyGuide. The format
 * is consistent everywhere; quality varies until each chapter is hand-
 * authored.
 *
 * Each chapter file exports exactly one `const X: RichChapterContent`.
 * The dynamic-import path picks that value out generically (default
 * export when present, otherwise the first own value on the module
 * namespace) so the registry stays book-name agnostic.
 */
...
/**
 * Keys of every hand-authored rich chapter. The Set is used by
 * `isHandAuthoredChapter` for synchronous existence checks (the admin
 * editor uses this to decide whether to skip the in-page editor) and
 * by `getRichChapter` to decide whether to dynamic-import the file.
 *
...
/**
 * In-process cache of resolved rich chapters. Production runs of the
 * Christ Index page aggregate every book, so the same chapter can be
 * requested multiple times in one render pass; caching avoids re-
 * resolving the same dynamic import.
 */
const RICH_CHAPTER_CACHE = new Map<string, RichChapterContent>();

/** Resolve a hand-authored chapter via dynamic import. Returns null if
 *  the key isn't in `RICH_CHAPTER_KEYS` or the module can't be loaded.
 *  Each module exports exactly one `RichChapterContent`; we pull it out
 *  by preferring `default` and falling back to the first own value on
...
async function loadRichChapter(
  ...
  if (!RICH_CHAPTER_KEYS.has(key)) return null;
  const cached = RICH_CHAPTER_CACHE.get(key);
  if (cached) return cached;
  try {
    const mod = (await import(`./${bookSlug}-${chapter}`)) as Record<string, unknown>;
    const candidate =
    ...
    RICH_CHAPTER_CACHE.set(key, candidate);
    return candidate;
  } catch {
    return null;
  }
...
/**
 * Look up a rich chapter, falling back to an auto-port of the legacy
 * data if no hand-authored chapter exists. Returns `null` only when
 * neither source has anything.
 *
 * `chapterVerses` — when supplied, the auto-port path renders the
 * entire chapter as a single scripture block at the top of the body so
 * readers still see the full chapter text, not just the key verse.
 *
 * Async because hand-authored chapters are loaded lazily — see
 * `loadRichChapter`. The synchronous existence check sits in
 * `isHandAuthoredChapter`.
 */
export async function getRichChapter(
  bookSlug: string,
  bookName: string,
  chapter: number,
  ...
  const rich = await loadRichChapter(bookSlug, chapter);
  if (rich) return rich;
  if (!legacyContent) {
...
/**
 * True when (book, chapter) has a hand-authored entry — the admin in-
 * page editor skips these because the rich layout doesn't round-trip
 * through the legacy ChapterContent shape (the file is the source of
 * truth). Synchronous: backed by `RICH_CHAPTER_KEYS`, no I/O.
 */
export function isHandAuthoredChapter(bookSlug: string, chapter: number): boolean {
  return RICH_CHAPTER_KEYS.has(`${bookSlug}/${chapter}`);
}

/** Build a scripture block that renders every verse of the chapter as
 *  plain lines (no highlights). Used by the auto-port path so readers
 *  always see the chapter text at the top of the guide. */
