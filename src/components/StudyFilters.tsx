...
export default function StudyFilters({
  ...
  // so the active pill reads without a saturated color background.
  const buttonClass = inline
    ? `frost-pill flex items-center gap-1.5 h-9 px-3.5 text-[0.75rem] font-semibold ${
        dd.open ? 'is-active' : ''
      }`
    : `frost-pill flex items-center gap-1.5 h-7 px-2.5 text-[0.6875rem] font-semibold ${
        dd.open ? 'is-active' : ''
      }`;
