    ...
    <div ref={ref} className="relative">
      ...
      >
        ...
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${
          isActive ? 'bg-[#007AFF]/[0.12]' : 'bg-[#F5F5F7]'
        }`}>
          <svg className={`w-4.5 h-4.5 ${isActive ? 'text-[#007AFF]' : 'text-[#86868B]'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="flex-1 text-left">
          ...
          <p className="text-[0.6875rem] text-[#AEAEB2] leading-snug">
            {isActive ? 'Viewing with denominational perspective' : 'Add a denominational perspective to your study'}
          </p>
        </div>

        {/* Chevron */}
        <svg className={`w-4 h-4 text-[#AEAEB2] transition-transform ${open ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {DENOMINATIONS.map((d) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-medium ${denomination === d.id ? 'text-[#007AFF]' : 'text-[#1D1D1F]'}`}>
                    {d.label}
                  </p>
                  <p className="text-[0.6875rem] text-[#AEAEB2] leading-snug truncate">
                    {d.description}
                  </p>
                </div>
              </button>
