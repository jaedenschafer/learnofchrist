#!/usr/bin/env node
/**
 * Ingest James Tissot's biblical watercolors into Supabase.
 *
 * Reads src/data/art/tissot.ts (70 curated plates from The Life of Our Saviour
 * Jesus Christ + Old Testament cycles, all verified on Wikimedia Commons).
 *
 * Usage:
 *   SUPABASE_SERVICE_ROLE_KEY=your_key node scripts/ingest-tissot.mjs
 *
...
function loadTissotData() {
  const path = resolve(__dirname, '../src/data/art/tissot.ts');
  const source = readFileSync(path, 'utf8');
  const cleaned = source
    .replace(/export type [\s\S]*?^};/m, '')
    .replace(/export const (\w+): [^=]+=/g, 'export const $1 =');
  const match = cleaned.match(/export const TISSOT_PLATES\s*=\s*(\[[\s\S]*?^\]);/m);
  if (!match) throw new Error('Failed to parse tissot.ts — expected TISSOT_PLATES array literal');
...
async function main() {
  ...
  console.log('=== James Tissot Biblical Cycles Ingest ===\n');

  const { plates } = loadTissotData();
  console.log(`Loaded ${plates.length} plates from tissot.ts`);

  console.log('\n→ Upserting artist: James Tissot');
  const [artist] = await upsert(
    ...
    [{
      ...
      death_year: 1902,
      nationality: 'French',
      bio: 'Jacques-Joseph (James) Tissot was a French painter and illustrator whose late-career devotional works — over 700 watercolors spanning the Old and New Testaments — became the most widely reproduced biblical cycle of the late 19th century.',
  ...
  for (const p of plates) {
    ...
    verifiedArtworks.push({
      ...
      license_note: 'Public domain via Wikimedia Commons. James Tissot (1836–1902), biblical watercolors from The Life of Our Saviour Jesus Christ (1886–1894) and The Old Testament.',
