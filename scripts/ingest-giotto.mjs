#!/usr/bin/env node
/**
 * Ingest Giotto di Bondone's Scrovegni Chapel fresco cycle into Supabase.
 *
 * Reads src/data/art/giotto.ts (39 narrative panels, 1303–1305, plus the
 * west-wall Last Judgment). All plates verified on Wikimedia Commons.
 *
 * Usage:
 *   SUPABASE_SERVICE_ROLE_KEY=your_key node scripts/ingest-giotto.mjs
 *
...
function loadGiottoData() {
  const path = resolve(__dirname, '../src/data/art/giotto.ts');
  const source = readFileSync(path, 'utf8');
  const cleaned = source
    .replace(/export type [\s\S]*?^};/m, '')
    .replace(/export const (\w+): [^=]+=/g, 'export const $1 =');
  const match = cleaned.match(/export const GIOTTO_PLATES\s*=\s*(\[[\s\S]*?^\]);/m);
  if (!match) throw new Error('Failed to parse giotto.ts — expected GIOTTO_PLATES array literal');
...
async function main() {
  ...
  console.log('=== Giotto Scrovegni Chapel Ingest ===\n');

  const { plates } = loadGiottoData();
  console.log(`Loaded ${plates.length} plates from giotto.ts`);

  console.log('\n→ Upserting artist: Giotto di Bondone');
  const [artist] = await upsert(
    ...
    [{
      ...
      death_year: 1337,
      nationality: 'Italian',
      bio: "Giotto di Bondone was a Florentine painter and architect whose frescoes in the Scrovegni (Arena) Chapel in Padua (1303–1305) are a watershed of Western art. His 39-scene Marian/Christological cycle and west-wall Last Judgment are the definitive medieval visual narrative of the gospel.",
      wikipedia_url: 'https://en.wikipedia.org/wiki/Giotto',
    }],
    'slug',
  );
  ...
  for (const p of plates) {
    ...
    verifiedArtworks.push({
      ...
      license_note: 'Public domain via Wikimedia Commons. Giotto di Bondone (c.1267–1337), Scrovegni (Arena) Chapel frescoes, Padua, 1303–1305.',
