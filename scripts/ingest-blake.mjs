#!/usr/bin/env node
/**
 * Ingest William Blake's biblical engravings and paintings into Supabase.
 *
 * Reads src/data/art/blake.ts (21-plate Book of Job engraving cycle +
 * standalone biblical paintings, all verified on Wikimedia Commons).
 *
 * Usage:
 *   SUPABASE_SERVICE_ROLE_KEY=your_key node scripts/ingest-blake.mjs
 *
...
function loadBlakeData() {
  const path = resolve(__dirname, '../src/data/art/blake.ts');
  const source = readFileSync(path, 'utf8');
  const cleaned = source
    .replace(/export type [\s\S]*?^};/m, '')
    .replace(/export const (\w+): [^=]+=/g, 'export const $1 =');
  const match = cleaned.match(/export const BLAKE_PLATES\s*=\s*(\[[\s\S]*?^\]);/m);
  if (!match) throw new Error('Failed to parse blake.ts — expected BLAKE_PLATES array literal');
...
async function main() {
  ...
  console.log('=== William Blake Biblical Ingest ===\n');

  const { plates } = loadBlakeData();
  console.log(`Loaded ${plates.length} plates from blake.ts`);

  console.log('\n→ Upserting artist: William Blake');
  const [artist] = await upsert(
    ...
    [{
      ...
      death_year: 1827,
      nationality: 'English',
      bio: 'William Blake was an English poet, painter, and printmaker whose visionary biblical illustrations — especially the 21-plate Illustrations of the Book of Job (1826) — profoundly shaped English-language religious imagery.',
  ...
  for (const p of plates) {
    ...
    verifiedArtworks.push({
      ...
      license_note: 'Public domain via Wikimedia Commons. William Blake (1757–1827), Illustrations of the Book of Job (engraved 1826) and related biblical paintings, tempera prints, and watercolors.',
