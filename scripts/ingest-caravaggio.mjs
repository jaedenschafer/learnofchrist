#!/usr/bin/env node
/**
 * Ingest Caravaggio's biblical canvases into Supabase.
 *
 * Reads src/data/art/caravaggio.ts (~32 canonical biblical works, c.1594–1610).
 * All plates verified on Wikimedia Commons.
 *
 * Usage:
 *   SUPABASE_SERVICE_ROLE_KEY=your_key node scripts/ingest-caravaggio.mjs
 *
...
function loadCaravaggioData() {
  const path = resolve(__dirname, '../src/data/art/caravaggio.ts');
  const source = readFileSync(path, 'utf8');
  const cleaned = source
    .replace(/export type [\s\S]*?^};/m, '')
    .replace(/export const (\w+): [^=]+=/g, 'export const $1 =');
  const match = cleaned.match(/export const CARAVAGGIO_PLATES\s*=\s*(\[[\s\S]*?^\]);/m);
  if (!match) throw new Error('Failed to parse caravaggio.ts — expected CARAVAGGIO_PLATES array literal');
...
async function main() {
  ...
  console.log('=== Caravaggio Ingest ===\n');

  const { plates } = loadCaravaggioData();
  console.log(`Loaded ${plates.length} plates from caravaggio.ts`);

  console.log('\n→ Upserting artist: Caravaggio');
  ...
  for (const p of plates) {
    ...
    verifiedArtworks.push({
      ...
      license_note: 'Public domain via Wikimedia Commons. Michelangelo Merisi da Caravaggio (1571–1610).',
